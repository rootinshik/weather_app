...
- Использовать **uv** как менеджер пакетов и виртуальных окружений (не pip, не poetry)
- **Никогда не редактировать `requirements.txt` вручную** — только генерировать командой `uv pip compile pyproject.toml -o requirements.txt` (или `uv pip freeze`)
- Каждый Python-образ (backend, bot, ml) имеет **собственный venv** — не делить окружения между сервисами

### Git-процесс
- Для каждого issue создаётся **отдельная ветка** (формат: `feature/issue-<номер>-краткое-описание`)
- Работа ведётся в ветке, мерж в main — через Pull Request
